...
public class AuthorMigrationStepConfig {
    ...
    public Step authorMigrationStep() {
        return new StepBuilder("authorMigrationStep", jobRepository)
                .<Author, CachedMigrationItem<AuthorDocument>>chunk(CHUNK_SIZE, transactionManager)
                .reader(authorReader())
    ...
    @Bean
    public ItemProcessor<Author, CachedMigrationItem<AuthorDocument>> authorProcessor() {
        return author -> new CachedMigrationItem<>(
                author.getId(),
